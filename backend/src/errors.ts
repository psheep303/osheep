...
export const errors = {
  ...
  parentNotFound: () => new ApiError(404, "PARENT_NOT_FOUND", "父目录不存在"),
  ...
  sessionNotFound: (id: string) => new ApiError(404, "SESSION_NOT_FOUND", `终端会话不存在: ${id}`),
  invalidSize: () => new ApiError(400, "INVALID_SIZE", "cols / rows 必须在 1..1000 之间"),
  ptySpawnFailed: (msg: string) => new ApiError(500, "PTY_SPAWN_FAILED", `PTY 启动失败: ${msg}`),
  tooManySessions: (limit: number) =>
    new ApiError(429, "TOO_MANY_SESSIONS", `并发会话数达到上限 ${limit}`),
  invalidQuery: (msg = "搜索参数非法") => new ApiError(400, "INVALID_QUERY", msg),
  notARepo: () => new ApiError(409, "NOT_A_REPO", "当前工作区不是 Git 仓库"),
  emptyCommitMessage: () => new ApiError(400, "EMPTY_COMMIT_MESSAGE", "commit 消息不能为空"),
  invalidRef: (msg = "ref 取值不合法") => new ApiError(400, "INVALID_REF", msg),
  ...
  branchExists: (msg = "分支已存在") => new ApiError(409, "BRANCH_EXISTS", msg),
  noUpstream: (msg = "当前分支未设置 upstream") => new ApiError(409, "NO_UPSTREAM", msg),
  ...
  upstreamFailed: (msg: string) => new ApiError(502, "UPSTREAM_FAILED", msg),
  codexCliNotFound: () => new ApiError(500, "CODEX_CLI_NOT_FOUND", "Codex CLI was not found"),
};
