...
const ICON_NAMES = ["icon.png", "icon.jpg", "icon.jpeg", "icon.webp", "icon.gif", "icon.svg"];
...
function opts(value: TemplateRegistryOptions = {}) {
  return {
    registryUrl: value.registryUrl ?? config.templateRegistryUrl ?? DEFAULT_REGISTRY_URL,
...
function parseRegistry(raw: unknown): TemplateRegistry {
  if (!raw || typeof raw !== "object") throw errors.ioError("template registry is invalid");
  ...
  for (const item of value.templates) {
    ...
    if (typeof source.path === "string" && source.path.trim() && !sourcePath) continue;
    templates.push({
      id: candidate.id,
      name: candidate.name,
...
async function fetchText(url: string, fetchImpl: typeof fetch): Promise<string> {
  ...
  if (!response.ok)
    throw errors.upstreamFailed(`template registry request failed (${response.status})`);
...
export function templateFileUrls(entry: TemplateRegistryEntry, file: string): string[] {
  const prefix = entry.source.path ? `${entry.source.path}/` : "";
  const encodedPath = `${prefix}${file}`.split("/").map(encodeURIComponent).join("/");
  const refs = [entry.version];
  if (/^\d+\.\d+\.\d+(?:[-+].*)?$/.test(entry.version)) refs.push(`v${entry.version}`);
...
export async function installRegistryTemplate(
  ...
  const entry =
    typeof entryOrId === "string"
      ? (await loadTemplateRegistry(settings)).templates.find((item) => item.id === entryOrId)
      : entryOrId;
  if (!entry)
  ...
  try {
    workflow = JSON.parse(new TextDecoder().decode(files.get("workflow.json"))) as Record<
      string,
      unknown
    >;
  } catch {
