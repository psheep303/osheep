...
- `agent/`
  - **已废弃（保留只读兼容）**：Agent 模块已下线，AI 对话直接使用 Provider + Model
  - 旧数据保留在此目录，新对话不再写入
  - 见 `docs/frontend/ai-panel.md` 的「Agent 模块下线」一节
...
5. AI 输出必须尽量结构化（统一系统提示词 + 标记块），便于前端展示和人工修订
6. AI 任务在用户关闭对话 Tab 后仍在后台运行（由前端 runtime 持有），重新打开能看到最新进度
