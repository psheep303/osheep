...
Linux 运行 `./dev.sh`，Windows 运行 `.\dev.ps1`。桌面版命令见 [README.md](README.md)。
...
- 提交即表示你按仓库的 MIT License 授权贡献，并遵守[行为准则](CODE_OF_CONDUCT.md)。
...
教程应短小并围绕任务展开；详细行为放到参考文档，不要堆进首次使用流程。简体中文是默认版本：中文用 `README.md`，英文用 `.en.md`。公开文档放入 `docs/`；本地笔记和生成内容放在 `.osheep/`，不要提交。
...
不要通过公开 Issue 或 PR 报告未修复漏洞。私密报告方式见 [SECURITY.md](SECURITY.md)。
