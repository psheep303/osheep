export function evaluateConditionExpression(
  ...
  const primary = (): unknown => {
    ...
    if (!token || [")", "&&", "||", "==", "!=", ">", "<", ">=", "<=", "!"].includes(token)) throw new Error("Expected a value.");
    return value(token);
  };
  const unary = (): unknown => tokens[index] === "!" ? (index++, !Boolean(unary())) : primary();
  const comparison = (): unknown => {
    ...
    if (operator === "==" || operator === "!=") {
      const equal = left === right || (!Number.isNaN(Number(left)) && !Number.isNaN(Number(right)) && Number(left) === Number(right));
      return operator === "==" ? equal : !equal;
    }
    const lhs = Number(left); const rhs = Number(right);
  ...
  const and = (): unknown => { let result = comparison(); while (tokens[index] === "&&") { index += 1; const right = comparison(); result = Boolean(result) && Boolean(right); } return result; };
  function or(): unknown { let result = and(); while (tokens[index] === "||") { index += 1; const right = and(); result = Boolean(result) || Boolean(right); } return result; }
  const result = or();
